...
pub async fn mw_ctx_resolve<B>(
  ...
  let ctx_ext_result = _cxt_resolve(mm, &cookies).await;
  ...
  // Store ctx_ext_result in the request extension
  req.extensions_mut().insert(ctx_ext_result);

  Ok(next.run(req).await)
}

async fn _cxt_resolve(mm: State<ModelManager>, cookies: &Cookies) -> CtxExtResult {
  ...
  // -- Get UserForAuth
  let user =
    UserBmc::first_by_username::<UserForAuth>(&Ctx::root_ctx(), &mm, &token.ident)
      .await
